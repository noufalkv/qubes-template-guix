...
| Executable build contracts | `tests/builder-hook-contract-check.sh`, `tests/builder-rpm-contract-check.sh`, `tests/rpm-layout-check.sh`, `tests/release-config-fragment-check.sh`, `make check` | Present; local run passes and exercises Builder hook outputs, Builder adapter RPM output, real template RPM layout, and the example release-config fragment shape |
...
| Tests should exercise real contracts | `tests/builder-hook-contract-check.sh`, `tests/builder-rpm-contract-check.sh`, `tests/rpm-layout-check.sh`, `tests/release-config-fragment-check.sh`, `VALIDATION.md` | Default local checks execute Builder hooks against a temporary install tree, feed generated images through the Builder RPM adapter, build/extract/reassemble RPM layouts, validate RPM lifecycle metadata, and parse the example release-config YAML; missing RPM/image tooling is a hard failure, not a false pass |
...
| Local tests cover real contracts | `make check` runs `tests/builder-hook-contract-check.sh`, `tests/builder-rpm-contract-check.sh`, `tests/rpm-layout-check.sh`, and `tests/release-config-fragment-check.sh`; these execute Builder hooks against a temporary install tree, package generated normal and minimal root images through the Builder adapter, build/extract/reassemble normal and minimal template RPM layouts, validate the RPM metadata path through the dom0 lifecycle harness in metadata-only mode, and parse the example release-config YAML. |
