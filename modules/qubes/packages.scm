          ...
          (replace 'install
            (lambda _
              ...
              (let ()
                ...
                (let ((xdg-icon
                       (string-append qubes-libdir "/xdg-icon")))
      ...
      #~(modify-phases %standard-phases
          ...
          (replace 'build
            (lambda _
              ...
              (setenv "CPPFLAGS"
                      (string-append
                       "-I" #$pipewire "/include/pipewire-0.3 "
                       "-I" #$pipewire "/include/spa-0.2 "
                       "-I" #$qubesdb-vm "/include "
                       "-I" #$qubes-libvchan-xen "/include/vchan-xen"))
              (setenv "LDFLAGS"
                      (string-append
                       "-L" #$qubesdb-vm "/lib "
                       "-L" #$pipewire "/lib "
                       "-L" #$qubes-libvchan-xen "/lib "
                       "-Wl,-rpath=" #$qubesdb-vm "/lib "
                       "-Wl,-rpath=" #$pipewire "/lib "
                       "-Wl,-rpath=" #$qubes-libvchan-xen "/lib"))
...
" port)))
    ...
    (description "PipeWire module that enables sound support in Qubes VMs.  It
...
" port)))))))
    ...
    (description "Install a desktop entry for xterm when the distribution package does not provide one.")
...
(define %qubes-common-packages
  (delete-duplicates
   (append %qubes-vm-gui-packages
           ...
           (list acpid adwaita-icon-theme conntrack-tools curl dbus e2fsprogs
                 font-alias font-misc-misc git glibc graphicsmagick guix
                 hicolor-icon-theme inetutils iproute kmod librsvg nano
                 nftables nss-certs python python-dbus python-pygobject
                 python-pyxdg setxkbmap socat sudo tango-icon-theme xdpyinfo
                 xev xinput xinit xmodmap xorg-server xprop xrandr xrdb
                 xsetroot xterm xterm-desktop-entry xwininfo zenity zstd))))
