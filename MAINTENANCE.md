...
- Qubes VM components are pinned in `qubes/vm.scm` by upstream tag, commit,
...
`guix system -L /etc/qubes-guix-channel reconfigure --no-bootloader --allow-downgrades /etc/config.scm`
